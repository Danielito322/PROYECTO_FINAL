(function(){
  const grid=document.getElementById('product-grid');if(!grid)return;let filter='Deporte';const categories=['Deporte','Belleza'];const chips=document.getElementById('shop-chips');
  chips.innerHTML=categories.map((c,i)=>`<button class="chip ${i===0?'active':''}" data-shop-filter="${c}">${c}</button>`).join('');
